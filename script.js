...
function halfx(){
  ...
  if (end > 0){
    ...
    statement = "Pick a damn number, zero does not count.";
  }
  alert(statement);
}

var fortuneButtonElement = document.getElementById("fortunebutton");
fortuneButtonElement.addEventListener("click", fortune);
...
function fortune() {
  ...
  var fortunefinal = fortunelist[Math.floor(Math.random() * fortunelist.length)];
  if(input){
    fortuneoutput.innerHTML = "To whom it may concern: " + fortunefinal;
  }
  }
...
function restyle(){
  ...
  var elBack3 = Math.floor(Math.random() * 57.5);
  var elBack2 = Math.floor(Math.random() * 57.5);


  fortuneoutput.style.fontSize = font + "px";
  fortuneoutput.style.color = "rgb(" + color1 + "," + color2 + "," + color3 + ")";
  fortuneoutput.style.backgroundColor = "rgb(" + elBack1 + "," + elBack2 + "," + elBack3 + ")";
}
